...
use vim_core_rs::{VimCoreSession, JobStatus, CoreCommandError, CoreHostAction};
...
fn test_inject_vfd_data_and_notify_status() {
    ...
    session.apply_ex_command("call job_start(['echo', 'hello'])").unwrap();
...
fn test_job_rejection_and_status() {
    ...
    session.apply_ex_command("let g:my_job = job_start(['echo', 'hello'])").unwrap();
    ...
    session.notify_job_status(job_id, JobStatus::Failed, -1).unwrap();

    session.apply_ex_command("call append(0, job_status(g:my_job))").unwrap();
    session.apply_ex_command("call append(1, job_info(g:my_job).exitval)").unwrap();

    let buf_id = session.snapshot().windows.iter().find(|w| w.is_active).unwrap().buf_id;
...
fn test_event_loop_interference() {
    ...
    session.apply_ex_command("let g:my_job = job_start(['sleep', '10'])").unwrap();

    // Verify Vim is not blocked and can still process UI events / commands
    session.apply_ex_command("call append(0, 'still alive')").unwrap();
    let buf_id = session.snapshot().windows.iter().find(|w| w.is_active).unwrap().buf_id;
    let buf_text = session.buffer_text(buf_id).unwrap();
    ...
    if let Some(id) = job_id {
        session.notify_job_status(id, JobStatus::Finished, 0).unwrap();
    }
}
...
fn test_in_memory_host_e2e_normal() {
    ...
    session.inject_vfd_data(vfd_out, b"hello from rust\n").unwrap();
    
    session.apply_ex_command("call append(0, 'JOB: ' . job_status(g:my_job))").unwrap();
    session.apply_ex_command("call append(1, 'CHAN: ' . ch_status(g:my_job))").unwrap();

    // Synchronously read from the channel in Vim script
    let read_result = session.apply_ex_command("let g:job_out = ch_readraw(g:my_job, {'part': 'out'})");
    if let Err(e) = &read_result {
        println!("READ ERR: {:?}", e);
    }

    // Finish the job
    session.notify_job_status(job_id, JobStatus::Finished, 0).unwrap();

    // Check variable content.
    session.apply_ex_command("call append(0, g:job_out)").unwrap();
