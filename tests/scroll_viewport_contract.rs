...
fn vertical_scroll_updates_viewport_boundaries() {
    ...
    assert!(!initial_snapshot.windows.is_empty(), "Window list should not be empty");
    ...
    assert!(initial_botline > 1 && initial_botline <= 100, "botline should be > 1");

    // Move cursor down by 50 lines which forces a scroll
    session.apply_normal_command("50j").expect("50j should succeed");
    ...
    assert!(scrolled_win.topline > 1, "topline should be > 1 after scrolling down");
    assert!(scrolled_win.botline > initial_botline, "botline should be advanced");
}
...
fn horizontal_scroll_updates_viewport_boundaries() {
    ...
    session.apply_ex_command("set nowrap").expect("set nowrap should succeed");
    ...
    session.apply_normal_command("200l").expect("200l should succeed");
    ...
    assert!(far_scrolled_win.leftcol > 0, "leftcol should increase after moving cursor far right with nowrap");
}
